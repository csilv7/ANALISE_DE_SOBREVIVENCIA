...
Simulou-se uma amostra proveniente de uma distribuição exponencial e, a partir dessa amostra, obteve-se a estimativa de máxima verossimilhança (EMV) para o parâmetro $\alpha$. Veja a @tbl-EMVexpSt, que apresenta as dez primeiras observações e suas respectivas funções de sobrevivência real e estimada.
...
# -------------------------
tempo <- rexp(n, rate = rate) # Simulação de tempos de sobrevivência
...
O valor verdadeiro do parâmetro é $\alpha =$ `{r} rate`. A estimativa de máxima verossimilhança obtida foi $\hat{\alpha} =$ `{r} round(emvExp, 2)`. Na @fig-CompEMVexp, comparamos graficamente as duas curvas de sobrevivência, ilustrando o valor real do parâmetro $\alpha$ e sua estimativa $\hat{\alpha}$.
...
ggplot(dados, aes(x = Tempo)) +
  ...
  scale_color_manual(
    ...
    labels = c(
      bquote(alpha == .(rate)),
      bquote(hat(alpha) == .(round(emvExp, 2)))
    )
  ) +
  labs(
    x = "Tempo",
...
```{r, warning=FALSE}
#| fig-cap: "Comparação do verdadeiro valor dos parâmetros $\\gamma$ e $\\alpha$ com suas estimativas de máxima verossimilhança."
...
ggplot(dfWeibull, aes(x = Tempo)) +
  ...
  scale_color_manual(
    ...
    labels = c(
      bquote("Verdadeiro: " ~ gamma == .(w_shape) ~ "," ~ alpha == .(w_scale)),
      bquote("EMV: " ~ hat(gamma) == .(round(estimate$par[1], 2)) ~ "," ~ hat(alpha) == .(round(estimate$par[2], 2)))
    )
  ) +
  labs(
    x = "Tempo",
...
```{r, warning=FALSE}
#| tbl-cap: "Dados de Sobrevivência Simulados com Tempos de Falha Censurados."
#| label: tbl-DATASIMULCENS
#| tbl-cap-location: top
...
gamma <- 3/2  # Parâmetro de forma da Weibull
alpha <- 1    # Parâmetro de escala da Weibull
exp.rate <- 1 # Parâmetro de taxa da exponencial
...
# Iteração de geração
for (i in 1:n) {
  t[i] <- rweibull(1, shape=gamma, scale=alpha) # Tempo de falha
  c[i] <- rexp(1, rate=exp.rate)                # Censura
  
  # Tempo de Falha observado
  times[i] <- min(t[i], c[i])
}
...
# Dados simulados
dados <- data.frame(times, delta)
prop.cens <- 1 - mean(dados$delta)

library(knitr)

knitr::kable(
  round(head(dados, 10), 4), 
  col.names = c("Tempo", "Censura"),
  escape = FALSE,
  align = 'c',
  booktabs = TRUE
)
...
Foram simulados `r n` tempos de falha, dentre eles, `r prop.cens * 100` são tempos de falha censurados. Deixa-se uma sugestão de variar essa proporção e avaliar a qualidade das estimativas. De posse dos dados simulados, gerou-se a @fig-HistSimulCens.


```{r, warning=FALSE}
#| fig-cap: "Histograma dos dados simulados."
#| label: fig-HistSimulCens
#| fig-cap-location: top

# Histograma
ggplot(data = dados, aes(x = times)) +
  geom_histogram(bins = 18, fill = "blue") +
  labs(x = "Tempo", y = "Frequência") +
  theme_minimal(base_size = 14)
```

Estimou-se a curva de sobrevivência pelo estimador de Kaplan-Meier, segundo uma distribuição exponencial e segundo uma distribuição Weibull. A @fig-CompCurvSobr mostra a comparação dessas estimativas.

```{r, warning=FALSE}
#| fig-cap: "Comparação das Curvas de Sobrevivência de Kaplan-Meier, Modelo Exponencial e Modelo Weibull."
#| label: fig-CompCurvSobr
#| fig-cap-location: top
...
ekm <- survfit(Surv(times, delta) ~ 1, data = dados)

# Formatando como DataFrame
data.km <- data.frame(Time=ekm$time, Survival=ekm$surv, Estimation="Kaplan-Meier")
...
# Função de Sobrevivência
st.exp <- function(t, alpha) exp(-alpha * t)

# EMV de α
emv.exp <- sum(dados$delta) / sum(dados$times)

# Formatando como DataFrame
data.exp <- data.frame(Time=dados$times, Survival=st.exp(dados$times, emv.exp), 
                       Estimation="Modelo Exponencial")
...
# Função de Sobrevivência
st.weib <- function(t, gamma, alpha) exp(-(t / alpha)^gamma)

# Função Log-verossimilhança
logWeibull <- function(par, dados){
  gamma <- par[1]   # Parâmetro de forma
  alpha <- par[2]   # Parâmetro de escala
  t <- dados$times  # Tempo de falha
  c <- dados$delta  # Variável indicadora
  
  ft <- dweibull(x=t, shape=gamma, scale=alpha)
  st <- 1 - pweibull(q=t, shape=gamma, scale=alpha)
...
# Formatando como DataFrame
data.weib <- data.frame(Time=dados$times, Survival=st.weib(dados$times, estimate$par[1], estimate$par[2]), 
                       Estimation="Modelo Weibull")
...
all.data <- rbind(data.km, data.exp, data.weib)

# Gráfico com ggplot2
ggplot(all.data, aes(x = Time, y = Survival)) +
  geom_line(aes(color = Estimation, lwd = Estimation)) +
  labs(x = "Tempo", y = "Probabilidade de Sobrevivência") +
  theme_minimal(base_size = 11) +
  theme(legend.position = "top",
        legend.text = element_text(size = 10)) +
  scale_color_manual(values = c("blue", "green", "red")) +
  scale_linewidth_manual(values = c(1.5, 1.5, 1.2))
```
